...
if (count($navItems) > 0) {
    ...
    foreach ($navItems as $ni) {
        $isColumn = $ni->cObj->getAttribute('navigation_column');
        $_class = '';
        if ($isColumn) { $_class = 'nav-column'; }

        echo '<li class="' . $ni->classes . '">'; //opens a nav item
        echo '<a href="' . $ni->url . '" target="' . $ni->target . '" class="' . $ni->classes . '">' . h($ni->name) . '</a>';

        if ($ni->hasSubmenu) {
            echo '<ul class="level-' . $ni->level . ' ' . $_class . '">'; //opens a dropdown sub-menu
            if ($ni->level === 1) {
                // echo $ni->level;
                echo '<li class="info"><div><h2>' . h($ni->name) . '</h2><a href="' . $ni->url . '">' . t('Read more'). '</a></div></li>';
            }
        } else {
