<?php namespace Application\Block\BlockMultiple;
...
class Controller extends BlockController
{
    ...
    protected $btExportTables = ['btBlockMultiple', 'btBlockMultipleItemEntries'];
    protected $btTable = 'btBlockMultiple';
    ...
    public function getSearchableContent()
    {
        ...
        $item_items = $db->fetchAll('SELECT * FROM btBlockMultipleItemEntries WHERE bID = ? ORDER BY sortOrder', [$this->bID]);
    ...
    {
        $db = Database::connection();
        $db->delete('btBlockMultipleItemEntries', ['bID' => $this->bID]);
        parent::delete();
    }
    ...
    public function duplicate($newBID)
    {
        ...
        foreach ($item_items as $item_item) {
            ...
            $db->insert('btBlockMultipleItemEntries', $item_item);
        }
  ...
  'relative_url',
        ...
        $al->register('css', 'repeatable-ft.form', 'blocks/block_multiple/css_form/repeatable-ft.form.css', [], $this->pkg);
        $al->register('javascript', 'handlebars', 'blocks/block_multiple/js_form/handlebars-v4.0.4.js', [], $this->pkg);
        $al->register('javascript', 'handlebars-helpers', 'blocks/block_multiple/js_form/handlebars-helpers.js', [], $this->pkg);
    ...
    public function save($args)
    {
        ...
        $rows = $db->fetchAll('SELECT * FROM btBlockMultipleItemEntries WHERE bID = ? ORDER BY sortOrder', [$this->bID]);
                    ...
                    switch ($type) {
                        case 'update':
                            foreach ($values as $id => $data) {
                                $db->update('btBlockMultipleItemEntries', $data, ['id' => $id]);
                            }
                            break;
                        case 'insert':
                            foreach ($values as $data) {
                                $db->insert('btBlockMultipleItemEntries', $data);
                            }
                            break;
                        case 'delete':
                            foreach ($values as $value) {
                                $db->delete('btBlockMultipleItemEntries', ['id' => $value]);
                            }
                            break;
                    }
                }
